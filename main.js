...
nav.innerHTML = NAV.map(n => ` <div class="nav-item ${n.screen === active ? 'active' : ''}" data-nav="${n.screen}"> <div class="nav-icon">${n.icon}</div> <div class="nav-label">${n.label}</div> </div>`).join(’’);
});
}
...
document.getElementById(‘slots-grid’).innerHTML = slots.map(s => ` <div class="slot ${s.available ? 'available' : ''}" data-time="${s.time}"> <span class="time">${s.time}</span> <span class="avail">${s.available ? 'open' : '–'}</span> </div>`).join(’’);
}
...
list.innerHTML = filtered.map(r => ` <div class="result-card"> <div class="result-dot dot-${r.status}"></div> <div class="result-body"> <div class="result-facility">${r.facility}</div> <div class="result-meta">${r.purpose} · ${r.date}<br>${r.time}</div> </div> <div class="result-tag tag-${r.status}">${labels[r.status]}</div> </div>`).join(’’);
}
...
return ` <div class="scan-card"> <div class="scan-dot ${dotClass}"></div> <div class="scan-body"> <div class="scan-name">${d.name}</div> <div class="scan-detail">${d.detail}</div> </div> <div class="scan-count ${countClass}">${countText}</div> </div>`;
}).join(’’);
}
